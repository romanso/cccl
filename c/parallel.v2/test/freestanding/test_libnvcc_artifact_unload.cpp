...
// that has nothing but the OS loader (CFE-104).
//
...
static bool run_cycle(const std::string& path, int* d_ptr, int expected)
{
  ...
  {
    std::fprintf(
      stderr, "artifact-unload: got %d, want %d, %s\n", result, expected, cudaGetErrorString(e));
    return false;
  }
...
int main()
{
  ...
  const std::filesystem::path dir =
    std::filesystem::temp_directory_path(ec) / ("libnvcc_artifact_" + std::to_string(
#if defined(_WIN32)
      GetCurrentProcessId()
#else
      getpid()
