...
// Several host objects are linked into one shared library and the single fatbin
// registration comes from a generated registration object (see libnvcc's
// buildRdcRegistrationObject). The header is force-included into every host TU, so
// emit the capture shim so that the per-TU copies MERGE into one instance instead
// of colliding at link, and keep the table exported so the loader can find it
...
//     atexit in the process (which would defer the dtor to process exit and
//     reintroduce the dangling-fatbin crash the unload logic prevents).
...
#    endif
  ...
  // The whole shared library has ONE fatbin and ONE registration object
  // (generated by wrapCudaBinary), whose constructor schedules exactly one
  // atexit(__cuda_module_dtor). The per-TU shim definitions here merge (weak /
  // selectany) into that single instance, so "one captured callback per library"
  // is the invariant and capacity 1 suffices. Overflow would mean more than one
  // registration constructor reached this shim -- an invariant violation -- so we
  // fail loudly (__builtin_trap below) instead of silently dropping an unregister,
  // which would reintroduce the dangling-fatbin crash the unload logic prevents.
  enum
  {
